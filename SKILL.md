...
First-time install lives in `install.md` (clone, deps, ffmpeg, skill registration, ASR credentials). Don't re-run it every session; on cold start just verify:

- `XFYUN_APP_ID` and `XFYUN_SECRET_KEY` resolve — either in the environment or in `.env` at the video-use repo root. If missing, ask the user to paste them and write them to `.env` (never to the user's `<videos_dir>`).
...
- **`transcribe.py <video>`** — single-file iFlytek long-form ASR call. `--num-speakers N` optional. Cached.
...
- **Running Whisper locally on CPU.** Slow and it normalizes fillers. Use hosted word-level ASR.
