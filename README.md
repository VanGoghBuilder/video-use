...
The agent handles the clone, dependencies, skill registration, and prompts you once for your iFlytek `APP_ID` and `SECRET_KEY`.
...
# 3. Add your iFlytek long-form ASR credentials
cp .env.example .env
...
**Layer 1 — Audio transcript (always loaded).** One iFlytek long-form ASR call per source gives word-level timestamps and speaker diarization. All takes pack into a single ~12KB `takes_packed.md` — the LLM's primary reading view.
